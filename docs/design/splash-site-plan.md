...
*Step 1a — Port JEDEE design-system updates.* **Done 2026-05-09** (`port-jedee-design-system` branch, commits `03c6eb9` → `34e88d7`). Schema and pipeline ported, colour values left at JEDEE defaults. See the **Files to port from JEDEE** inventory below for what landed.

*Step 1b — Apply the Tolstoy period palette.* **Done 2026-05-09** (`port-jedee-design-system` branch, commit `9a9fe0d`). Four-period palette in `colorsBase.json`, four-stop warm parchment / slate charcoal surface ramp, per-period dark-theme tints, Orange slot renamed to Amber, build-script rebrand JEDEE → tolstoy.life, prose consolidated from `src/_design/design-system.md` into `DESIGN.md` body. The three open shade questions (amber vs gold, forest vs spring, dark-theme accent strategy) are answered in [docs/design/period-colours-research.md "Decisions landed — 2026-05-09" section](design/period-colours-research.md). Penpot import (1e) still pending — it's a manual Johan step.
...
*Step 1d — Run the pipeline.* `npm run colors` (regenerates `colors.json`), then `npm run penpot:tokens` (regenerates `tokens/penpot-tokens.dtcg.json` and `DESIGN.md`). **Note:** 1d ran as part of 1a + 1b; only re-run after 1c lands new font values.

*Step 1e — Penpot import.* Manual import of the DTCG file into the tolstoy.life Penpot file. Set theme set membership manually if Light/Dark themes import empty. **Pending** — `website/tokens/penpot-tokens.dtcg.json` is current as of `9a9fe0d` (92 tokens, Tolstoy-branded).
...
- **Tolstoy-only (already present, don't touch from JEDEE):** `src/_layouts/{wiki,work}.njk`, `src/{wiki,works,sources,_staging}/`, `schema/`, `.github/`, `src/pages/{works.njk,changelog.md}`. (Note: `src/_design/` was removed in 1b — its prose lives in `DESIGN.md`'s body now.)
