...
- [x] **Kreutzer Sonata novel-dive shipped** (2026-06-08) — 2nd flagship novel-dive, pushed (parent `d104a629` + pointer `d8b5ce99`; submodule note `70703a7`). The Afterword got its **own** record-creating `workRecord` (companion-text precedent). verify_quotes PASS 37/37; opus verifier 0 blocking. **Next novel-dive: Hadji Murat** — scope handoff ready at `_generated/sessions/2026-06-08-hadji-murat-dive-scope-handoff.md`. Queue after: Father Sergius, Master and Man, The Power of Darkness.
- [ ] **Decide the `character`/`group` wiki-schema question** (now blocking THREE novel-dives: Resurrection, Kreutzer, Hadji Murat) — add a `character` (+ probably `group`) wiki type, OR fold a novel's principals into the work's own page. Per [[feedback_explain_schema_before_edit]], gated before ingesting any novel-dive's entities; also blocks the `--novel` mode build. Fictional characters currently routed as `concept` stopgaps in each dossier's `needsReview`.
