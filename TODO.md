...
- [ ] **The Great Sin — regenerate English audio for Part II & IX** — the 3 English broken-sentence repairs shifted sentence boundaries in those two chapters, so the audio + EPUB in `build/` is stale until re-run (separate `projects/audiobook` repo). `build/` is regenerable/gitignored; the current build keeps working until then.
