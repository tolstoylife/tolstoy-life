...
- [x] **Prophet-period dive backlog cleared** (2026-06-12) — the four remaining work-dives all shipped: Круг чтения weekly tales, After the Ball, The Forged Coupon, and **The Living Corpse** (13th dive, `docs/research/1900-the-living-corpse/`; novel mode + drama flex; parent `f396d772`, website note `cc6e017`; pushed). Living Corpse marquee = `confirms`+`complicates`+`extends` (the institution is the only villain; the abandonment was over-determined — in his own diary the conscience-turn against art-for-amusement, not the Gimer plea, is dominant; he was «возмутился»/provoked, not charmed, by *Uncle Vanya*); record-creating `workRecord` (plays/drama; `bans: []` author-WITHHELD); verify_quotes 40/40, verifier CLEAN-WITH-MINORS. **The Prophet period (1880–1910) is now fully dived** — all major non-fiction, all major fiction, all three dramas, both folk-tale clusters. The paste-ready planning file `docs/research/_prophet-period-remaining-dives.md` (untracked) is fully consumed. Natural next steps: the cross-dive `needsReview` batch-review (item below) and/or LLM wiki ingestion from the 13 dossiers.
...
- [ ] **The Great Sin — Phase 3 remainder** — (a) Johan's own annotations on the dive (the critique that further refines the overview); (b) the **entity steer** — reading-derived steer for the wiki entities (Isabella Fyvie Mayo [new], Henry George [enrich — his faith/religion language], the "religion" + "political economy" concepts; loose-match the vault before any `missing`). The dossier `entities:` block is the routing map; only Chertkov currently exists in `website/src/wiki/`.
